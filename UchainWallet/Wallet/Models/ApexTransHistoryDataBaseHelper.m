...
{
    static ApexTransHistoryDataBaseHelper *manager = nil;
...
- (void)addTransferHistory:(ApexTransferModel*)model forWallet:(NSString *)walletAddress manager:(id<ApexTransHistoryProtocal>)manager {
    ...
    while ([res next]) {
        ...
        if ([txidInDB isEqualToString:model.txid] && status == ApexTransferStatus_Confirmed) {
        ...
        if (status == ApexTransferStatus_Progressing || status == ApexTransferStatus_Blocking) {
