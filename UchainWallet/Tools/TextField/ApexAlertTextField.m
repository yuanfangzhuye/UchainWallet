...
- (void)setUI{
    self.bottomLine = [UchainUtil addLineInView:self color:[UchainUtil grayColor] edge:UIEdgeInsetsMake(-1, 0, 0, 0)];
}
    ...
    if (self.alertShowConditionBlock) {
       self.isAlertShowing = self.alertShowConditionBlock(self.text);
        ...
        if (!self.isAlertShowing) {
            self.bottomLine.backgroundColor = [UchainUtil grayColor];
        }else{
